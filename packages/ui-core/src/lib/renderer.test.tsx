...
describe("Renderer with dataSource", () => {
  ...
  it("logs and renders nothing when dataSource method is not in acceptsDataFrom", () => {
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    // DataComponent only accepts "searchTracesPage" per _testCatalog; feed it
    // a "searchLogsPage" dataSource to trigger the misconfig branch.
    const tree = {
      root: "data-1",
      elements: {
        "data-1": {
          ...
          children: [],
          parentKey: "",
          dataSource: { method: "searchLogsPage", params: {} },
        },
      },
    } satisfies UITree;

    const Wrapper = createWrapper(mockClient);
    const { container } = render(createElement(TestRenderer, { tree }), {
      wrapper: Wrapper,
    });
    ...
    expect(msg).toContain("DataComponent");
    expect(msg).toContain("searchLogsPage");
    expect(msg).toContain("searchTracesPage");
